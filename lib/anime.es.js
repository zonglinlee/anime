...
function spring(string, duration) {
  ...
  var mass = minMaxValue(is.und(params[0]) ? 1 : params[0], .1, 100);
  var stiffness = minMaxValue(is.und(params[1]) ? 100 : params[1], .1, 100);
  var damping = minMaxValue(is.und(params[2]) ? 10 : params[2], .1, 100);
  var velocity =  minMaxValue(is.und(params[3]) ? 0 : params[3], .1, 100);
...
function elastic(amplitude, period) {
  ...
  var a = minMaxValue(amplitude, 1, 10);
  var p = minMaxValue(period, .1, 2);
  return function (t) {
    return (t === 0 || t === 1) ? t : 
...
var easings = (function () {
  ...
  var loop = function ( equation ) {
    penner[equation].forEach(function (ease, i) { 
      eases['ease'+equation+names[i]] = ease;
    });
  };

  for (var equation in penner) loop( equation );
...
function parseEasings(tween) {
  var string = tween.easing;
  var name = string.split('(')[0];
  var args = parseEasingParameters(string);
  var ease = easings[name];
  switch (name) {
...
function getStates(params) {
  var statesParams = params.states;
  if (!statesParams) { return; }
  var instanceParams = cloneObject(params);
  delete instanceParams.states;
  var states = {default: instanceParams};
  for (var state in statesParams) {
    states[state] = cloneObject(instanceParams);
    var stateParams = statesParams[state];
...
function getInstanceTimings(animations, tweenSettings) {
  ...
  timings.delay = animLength ? Math.min.apply(Math, animations.map(function (anim) { return anim.delay; })) : tweenSettings.delay;
  timings.endDelay = animLength ? timings.duration - Math.max.apply(Math, animations.map(function (anim) { return anim.duration - anim.endDelay; })) : tweenSettings.endDelay;
  return timings;
}
...
function anime(params) {
  ...
  function syncInstanceChildren(time) {
    if (time >= instance.currentTime) {
      for (var i = 0; i < childrenLength; i++) { children[i].seek(time - children[i].timelineOffset); }
    } else {
      for (var i$1 = childrenLength; i$1--;) { children[i$1].seek(time - children[i$1].timelineOffset); }
    }
  }
  ...
  instance.animateTo = function(stateName, paramsOverrides) {
    var state = instance.states[stateName];
    if (!state) { return; }
  ...
  instance.goTo = function(stateName, paramsOverrides) {
    var stateAnimation = instance.animateTo(stateName, paramsOverrides);
