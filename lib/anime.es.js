    ...
    if (!values.length) {
      for (var index = 0; index < t; index++) {
        if (!grid) {
          ...
          var x = !fromCenter ? fromIndex%grid[0] : (grid[0]-1)/2;
          var y = !fromCenter ? Math.floor(fromIndex/grid[1]) : (grid[1]-1)/2;
          var dx = x - index%grid[0];
          var dy = y - Math.floor(index/grid[1]);
