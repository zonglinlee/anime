...
(function (global, factory) {
  ...
  function filterArray$1(arr, callback) {
  ...
  function flattenArray$1(arr) {
    return arr.reduce((a, b) => a.concat(is.arr(b) ? flattenArray$1(b) : b), []);
  }
  ...
  function parseTargets(targets) {
    const targetsArray = targets ? (flattenArray$1(is.arr(targets) ? targets.map(toArray) : toArray(targets))) : [];
    return filterArray$1(targetsArray, (item, pos, self) => self.indexOf(item) === pos);
  }
  ...
  function getAnimations(animatables, properties) {
    return filterArray$1(flattenArray$1(animatables.map(animatable => {
  ...
  function anime(params = {}) {
    ...
    function setAnimationsProgress(insTime) {
      ...
      while (i < animationsLength) {
        ...
        if (tweenLength) tween = filterArray$1(tweens, t => (insTime < t.end))[0] || tween;
